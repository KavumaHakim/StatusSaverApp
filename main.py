...
class ImageScreen(Screen):
    ...
    async def async_load_images(self, image_list):
        ...
        for image in image_list:
            preview = ImageCard()
            img = Image(source=image, pos_hint={"center_x": 0.5, "center_y": 0.5})
...
class VideoScreen(Screen):
    ...
    def generate_thumbnail(self, video, timestamp=0.2):
        cap = cv2.VideoCapture(video)
        cap.set(
            cv2.CAP_PROP_POS_MSEC, timestamp * 1000
        )  # Setting thumbnail to 'timestamp' seconds in.
    ...
    async def async_load_thumbnails(self, video_list):
        ...
        for index, video in enumerate(video_list):
            ...
            if video_id in added_video_ids:
                continue
            thumbnail = Image(pos_hint={"center_x": 0.5, "center_y": 0.5})
            ...
            if self.ids["save_delete_icon"].icon == "download":
                ...
                for saved_path in video_paths_saved:
                    if os.path.getsize(saved_path) == viewed_vid_size:
                        with open(saved_path, "rb") as vs, open(
                            self.video_source, "rb"
                        ) as source_vid:
